...
def validate_site(data):
    ...
    for idx, channel in enumerate(channels, 1):
        ...
        if freq in freqs:
            raise SystemExit(f"duplicate channel frequency: {freq}")
        freqs.append(freq)
        ...
        sink = pulse_sink(channel, idx - 1)
        if sink in pulse_sinks:
            raise SystemExit(f"duplicate pulse sink: {sink}")
        pulse_sinks.add(sink)

        suffix = output_suffix(channel)
        if suffix in suffixes:
            raise SystemExit(f"duplicate output suffix: {suffix}")
        suffixes.add(suffix)
...
def target_mapping(data, config_dir=DEFAULT_CONFIG_DIR):
    ...
    if data.get("site", {}).get("rclone_remote"):
        mapping["sync.env"] = Path(config_dir) / "sync.env"
    for channel in data["channels"]:
        mapping[f"{channel_name(channel)}.env"] = Path(config_dir) / f"{channel_name(channel)}.env"
    return mapping


def obsolete_targets(data, config_dir=DEFAULT_CONFIG_DIR):
    config_dir = Path(config_dir)
    old = set(old_channels(config_dir))
    new = set(new_channels(data))
    mapping = {f"{channel}.env": config_dir / f"{channel}.env" for channel in sorted(old - new)}
    if not data.get("site", {}).get("rclone_remote"):
        mapping["sync.env"] = config_dir / "sync.env"
    return mapping
...
def show_diff(data, generated, config_dir=DEFAULT_CONFIG_DIR):
    ...
    for dest in obsolete_targets(data, config_dir).values():
        show_removed_file(dest)
...
def show_plan(data, config_dir=DEFAULT_CONFIG_DIR):
    old = set(old_channels(config_dir))
    new = set(new_channels(data))
    removed_targets = obsolete_targets(data, config_dir)
    ...
    if data.get("site", {}).get("rclone_remote"):
        ...
        print("- Disable train-recorder-sync.timer and remove sync.env for local-only mode")
    for name, dest in removed_targets.items():
        if Path(dest).exists():
            print(f"- Remove stale generated file {dest}")
...
def remove_with_backup(dest, backup_dir):
    dest = Path(dest)
    if not dest.exists():
        return
    if not dest.is_file():
        raise SystemExit(f"refusing to remove non-file path during apply: {dest}")
    backup_dir.mkdir(parents=True, exist_ok=True)
    shutil.copy2(dest, backup_dir / dest.name)
    dest.unlink()
...
def apply_config(data, generated, yes=False, config_dir=DEFAULT_CONFIG_DIR, install_dir=DEFAULT_INSTALL_DIR):
    ...
    removed_targets = obsolete_targets(data, config_dir)
    backup_mapping = dict(mapping)
    backup_mapping.update(removed_targets)
    ...
    try:
        ...
        for dest in removed_targets.values():
            remove_with_backup(dest, backup_root)

        template = Path(install_dir) / "Service_Files" / "vox@.service"
        if template.exists():
            copy_with_backup(template, SYSTEMD_DIR / "vox@.service", backup_root)
            backup_mapping["vox@.service"] = SYSTEMD_DIR / "vox@.service"
        ...
        if data.get("site", {}).get("rclone_remote"):
            ...
            run(sudo + ["systemctl", "disable", "--now", "train-recorder-sync.timer"], check=False)
