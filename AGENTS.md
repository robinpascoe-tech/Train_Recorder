...
- `Scripts/site_config.sh` is a shell entry point; the maintainable implementation now lives in `Scripts/site_config.py` with unit tests under `tests/`. Keep behavior changes in the Python module unless the shell wrapper itself needs adjustment.
