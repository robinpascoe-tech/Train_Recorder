...
class WifiCheckTests(unittest.TestCase):
    def test_check_rclone_uses_run_user_without_sudo(self):
        ...
        with mock.patch.object(wifi_check, "command_exists", return_value=True), \
    ...
    def test_collect_preserves_previous_failure_after_recovery(self):
        ...
        previous = {"last_failure": {"generated_at": "2026-06-18T14:00:00+00:00", "summary": "old failure"}}

        with mock.patch.object(wifi_check, "runtime_env", return_value={}), \
            ...
            mock.patch.object(wifi_check, "wifi_status", return_value={"ssid": "ONR", "source": "iwgetid", "connected": True}), \
    ...
    def test_collect_runs_remedy_when_failures_present(self):
        ...
        with mock.patch.object(wifi_check, "runtime_env", return_value={}), \
            ...
            mock.patch.object(wifi_check, "wifi_status", return_value={"ssid": "", "source": "", "connected": False}), \
            mock.patch.object(wifi_check, "ip_addresses", return_value=[]), \
            mock.patch.object(wifi_check, "check_gateway", return_value=failure), \
            mock.patch.object(wifi_check, "check_dns", return_value={"name": "dns", "ok": True, "message": "ok", "details": {}}), \
            mock.patch.object(wifi_check, "check_rclone", return_value={"name": "rclone", "ok": True, "message": "ok", "details": {}}), \
            mock.patch.object(wifi_check, "check_dashboard", return_value={"name": "dashboard", "ok": True, "message": "ok", "details": {}}), \
            mock.patch.object(wifi_check, "remedy_actions", return_value=[{"action": "wpa_cli reconnect", "ok": True}]), \
            mock.patch.object(wifi_check.platform, "node", return_value="recorder"):
