...
RailWave Pi is the product name for this project. Installed paths, generated config directories, and systemd unit names still use the existing `train-recorder` identifiers for compatibility with deployed systems.
...
`site_config.sh` manages site-specific generated configuration. It is a small shell wrapper over `site_config.py`, which holds the testable Python implementation for parsing, generation, plan, diff, apply, and doctor dispatch. It requires PyYAML (`python3-yaml` on Debian/Raspberry Pi OS) so `/etc/train-recorder/site.yaml` is always parsed with a real YAML parser. It can run an interactive wizard, generate `rtl_airband.conf`, `system.pa`, `common.env`, `sync.env`, and channel env files from `site.yaml`, show an apply plan, show redacted diffs, reconcile live `vox@...` services, and run the doctor checks. `apply` is the activation step for generated installs: it enables/restarts PulseAudio, RTLSDR-Airband, the configured recorder services, and the train-recorder timers. It also removes stale generated channel env files and disables the sync timer when `rclone_remote` is removed so the live config converges back to local-only mode. The YAML file is the desired state; generated files are artifacts. If `site.yaml` already exists, the wizard uses it as the default source for prompts and preserves sensitive Broadcastify values without printing them. Per-channel RTLSDR-Airband options such as `squelch_snr_threshold`, `squelch_threshold`, `bandwidth`, and `ctcss` live in each `channels` entry and are generated into the matching `rtl_airband.conf` channel block.

`rtl_airband.service` starts RTLSDR-Airband and reads the installed RTL-Airband config. It uses `Restart=on-failure` with a 30 second delay and systemd start-rate limiting, so transient SDR or PulseAudio disruption gets a few recovery attempts without hiding a persistent bad config or missing dongle. It is also `PartOf=pulseaudio.service`, so an explicit PulseAudio restart recycles RTLSDR-Airband.

`pulseaudio.service` starts PulseAudio in system mode. The included `system.pa` creates two null sinks named `myfreq1sink` and `myfreq2sink`; their `.monitor` sources are used by SOX. It uses `Restart=on-failure` with one delayed retry in a 10 minute start-limit window. Recorder appliances should not also run the per-user `pi` PulseAudio service/socket; the installer can mask those user units so SOX and RTLSDR-Airband consistently use the system-mode server.
